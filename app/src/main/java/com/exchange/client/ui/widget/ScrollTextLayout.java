...
public class ScrollTextLayout extends LinearLayout {
    ...
    private boolean mShowDividers = true;
    private int mDividerPadding;

    public ScrollTextLayout(Context context, int themeAttr) {
        super(context);

        TypedArray a = context.obtainStyledAttributes(null, LL, themeAttr, 0);
        setDividerView(a.getDrawable(ScrollTextLayout.LL_DIVIDER));
        mDividerPadding = 0;
        a.recycle();
    }

    public void setDividerView(Drawable divider) {
        if (divider == mDivider) {
            return;
        }
        mDivider = divider;
